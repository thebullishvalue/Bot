...
logger = logging.getLogger("pragyam.metrics")
...
class ExecutionMetrics:
    ...
    def print_summary(self):
        ...
        # Timing summary
        logger.info(f"Total Duration: {self.get_total_duration():.2f}s")
        for phase_name, phase in self.phases.items():
            status_icon = "✓" if phase.status == "success" else "✗"
            logger.info(f"  {phase_name}: {phase.duration:.2f}s [{status_icon}]")

        # Counters
        logger.info(f"Symbols: {self.symbols_count}")
        logger.info(f"Days: {self.days_count}")
        logger.info(f"Strategies: {self.strategies_count}")
        logger.info(f"Portfolios Generated: {self.portfolios_generated}")
        logger.info(f"Rebalances: {self.rebalances}")

        # Performance
        if self.total_return != 0:
            logger.info(f"Total Return: {self.total_return*100:.2f}%")
...
def reset_metrics():
    """Reset the global metrics instance for a new run."""
    global _metrics_instance
    _metrics_instance = ExecutionMetrics()
